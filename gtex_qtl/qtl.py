...
def call_qtls(expression_df, gene_window_indexes, vcf_path,
        covariates_df, gx_covariates_df=None,
    ...
    Args:
        ...
        vcf_path: path to the BGZIP-compressed vcf containing the genotypes in
            "GT" format.
        covariates_df: covariate data frame used to residualize genotype, and
            gene expression if `gx_covariates_df` is not provided. First column
            is interpreted as covariate identifier, all other columns as values
            for the sample of matching column name.
        gx_covariates_df: different set of covariates to use specifically for
            gene expression. If not given, defaults to the same `covariates_df`
            used to residualize genotype.
